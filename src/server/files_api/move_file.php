...
/**
 * Moves a file directly from source path to target path.
 *
 ...
 */
function move_file(mixed $oldPath, mixed $newPath, mixed $overwrite, mixed $token): string {
    ...
    if (!$overwriteBool && file_exists($absoluteNew) && $absoluteOld !== $absoluteNew)
    ...
    if (!rename($absoluteOld, $absoluteNew)) {
        http_error(500, "Failed to move file from $relativeOld to $relativeNew");
    }

    return "Successfully moved file from $relativeOld to $relativeNew";
}
