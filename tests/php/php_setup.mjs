...
import { mkdir, mkdtemp, symlink, unlink } from "fs/promises";
...
async function setup_php() {
	...
	const jsonCopyProms = jsonPaths.map((from) => {
		...
		const copyFiles = /** @type {(from: string, to: string) => Promise<unknown>} */ copy;
		return copyFiles(from, to).then((res) => {
